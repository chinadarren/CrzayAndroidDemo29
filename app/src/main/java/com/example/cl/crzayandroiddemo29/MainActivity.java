package com.example.cl.crzayandroiddemo29;
...
/**
 * ScreenNo保存当前正在显示第几屏的程序列表。
 * BaseAdapter对象会根据screenNo动态计算该Adapter总共包含多少个列表项。
 */
...
public class MainActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState)  {
        ...
        button = (Button) findViewById(R.id.button_prev);
    ...
    public void prev(View v){
        if(screenNo > 0){
            screenNo -- ;
            //为ViewSwitcher的组件显示过程设置动画
            switcher.setInAnimation(this,R.layout.slide_out_left);
            //为ViewSwitcher的组件隐藏过程设置动画
           // switcher.setOutAnimation(this,R.layout.slide_in_right);
            ((GridView)switcher.getNextView()).setAdapter(adapter);
            switcher.showNext();
        }
    }
    ...
    private BaseAdapter adapter = new BaseAdapter()  {
        ...
        public int getCount() {
            ...
            //负责每屏显示的程序数为应用程序的数量对NUMBER_PER_SCREEN
            return NUMBER_PER_SCREEN;
        }

        @Override
        public DataItem getItem(int position) {
            //根据screenNo计算第Position个列表的数据
